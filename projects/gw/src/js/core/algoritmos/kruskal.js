...
(function(){
    ...
    Grafo.Algoritmos.Kruskal = function() {
        if(Grafo.Direcionado) {
            console.log('Directed graphs are not supported.');
            return 0;
        }
        console.log('Kruskal\'s has started...');
        ...
        for(i = 0; i < numVertices; i++)
            for(j = i +1; j < numVertices; j++)
                if(Grafo.MatrizAdj[i][j] > 0)
                    lsAresPrioridade.push({v1: i, v2: j, peso: Grafo.MatrizAdj[i][j]});
        ...
        while(numAresColocadas < numVertices -1 || lsAresPrioridade.length) {
            ...
            console.log('Lighter edge found: [' +arestaAtual.v1 +'] - [' +arestaAtual.v2 +']');
            ...
            console.log('Set A: ' +ConjContemA +'. Set B: ' +ConjContemB);
        ...
        App(true);      // reinicia a aplicacao, agora buscando a matri de adjacencia a qual
