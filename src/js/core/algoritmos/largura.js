...
(function() {
    ...
    var Largura = function() {
        if (Grafo.Direcionado) {
            lnConsole.message('Desculpe, mas Largura ainda n�o foi implementado para grafos direcionados!')
            return false
        }
        
        console.log('Inicio do algoritmo de busca em largura')        
        lnConsole.message('Inicio da busca em largura')
		...
		while (cinzas.length > 0) {
            ...
            lnConsole.message('Vertice em an�lise: ' +(vertAtual +1))
			...
			for (i = 0; i < Grafo.MatrizAdj.length; i++)
				...
				if (Grafo.MatrizAdj[vertAtual][i] != 0 && vertices[i].cor == 0) {
					...
					lnConsole.message(' Filho branco: ' +(i +1) +' no tempo ' +tempo)
				}
			
			vertices[vertAtual].cor = 2
			vertices[vertAtual].fim = ++tempo
			console.log('Tempo de sa�da: ' +tempo)
			lnConsole.message('Tempo de sa�da: ' +tempo)
		}
